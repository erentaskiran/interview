...
const CompButton = () => (
  <CompFrame kicker="Component" title="Button">
    <CompSection label="Primary">
      <Button kind="primary" icon="sparkle">
        Start interview
      </Button>
      ...
      <Button kind="primary" size="lg" iconRight="arrow">
        Start now
      </Button>
    </CompSection>
    <CompSection label="Accent · used on voice flow">
      <Button kind="accent" icon="mic">
        Record answer
      </Button>
      <Button kind="accent" size="sm" icon="play">
        Play
      </Button>
    </CompSection>
    <CompSection label="Ghost & quiet">
      <Button kind="ghost" icon="upload">
        Import
      </Button>
    ...
    <CompSection label="Danger & disabled">
      <Button kind="danger" icon="stop">
        End session
      </Button>
      <button className="btn btn--primary" style={{ opacity: 0.45, pointerEvents: "none" }}>
        Submit
      </button>
    </CompSection>
  </CompFrame>
  ...
  <CompFrame kicker="Component" title="Input">
    ...
    <Field label="Notes">
      <textarea
        className="textarea"
        rows="3"
        defaultValue="Focus on STAR framing. Push for measurable outcomes when missing."
        readOnly
      />
    </Field>
  </CompFrame>
...
const TemplateCard = ({ title, author, category, likes, range, liked, hot }) => (
  ...
  >
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      <Chip kind={category === "Engineering" ? "accent" : category === "Product" ? "ok" : null} dot>
        {category}
      </Chip>
      {hot && (
        <span className="micro" style={{ color: "var(--acc-deep)" }}>
          ↑ trending
        </span>
      )}
    </div>
    ...
    <div
      className="small"
      style={{ display: "flex", alignItems: "center", gap: 8, color: "var(--ink-600)" }}
    >
      <Avatar
        name={author
          .split(" ")
    ...
    >
      ...
      <span
        style={{
          ...
          alignItems: "center",
          gap: 4,
          color: liked ? "var(--err)" : "var(--ink-600)",
        }}
      >
        <Icon
          name="heart"
...
const CompTemplateCard = () => (
  <CompFrame kicker="Component" title="TemplateCard">
    <div className="body">
      Compact card used in Discovery, profile, search results. Default · liked · trending states.
    </div>
  ...
  return (
    <div
      ...
      style={{
        ...
        flexDirection: "column",
        gap: 14,
        background: state === "recording" ? "var(--ink-900)" : "var(--surface)",
      }}
    >
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <span
          ...
          style={{
            fontSize: 11,
            color: state === "recording" ? "var(--surface)" : "var(--ink-600)",
            opacity: state === "recording" ? 0.8 : 1,
        ...
        <span
          ...
          style={{
            marginLeft: "auto",
            fontSize: 11,
            color: state === "recording" ? "var(--surface)" : "var(--ink-500)",
            opacity: state === "recording" ? 0.6 : 1,
          }}
        >
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        {state === "idle" && (
          <>
            <Button kind="accent" icon="mic" size="lg" block>
              Record answer
            </Button>
          </>
        )}
        {state === "recording" && (
          <>
            <Button kind="danger" icon="stop" size="lg" block>
              Stop
            </Button>
            <button
        ...
        {state === "review" && (
          <>
            ...
            <Button kind="ghost" icon="mic">
              Re-record
            </Button>
            <Button kind="primary" iconRight="arrow" style={{ marginLeft: "auto" }}>
              Submit
            </Button>
          </>
        )}
...
const CompAudioRecorder = () => (
  <CompFrame kicker="Component" title="AudioRecorder">
    <div className="body">
      Four states. Recording flips to inverted ink surface so it reads as "live" anywhere on a page.
    </div>
...
const CompSessionProgress = () => (
  <CompFrame kicker="Component" title="SessionProgress">
    <div className="body">
      Adaptive count made visible: filled dots = answered, ring dot = active, the small notch marks
      the AI's earliest finish point.
    </div>
    <CompSection label="Just started">
      <div style={{ width: "100%" }}>
        <SessionProgress now={1} planned={6} />
      </div>
    </CompSection>
    <CompSection label="Mid session">
      <div style={{ width: "100%" }}>
        <SessionProgress now={4} planned={6} />
      </div>
    </CompSection>
    <CompSection label="Past minimum — AI could finish">
      <div style={{ width: "100%" }}>
        <SessionProgress now={5} planned={6} />
      </div>
    </CompSection>
    <CompSection label="At hard cap">
      <div style={{ width: "100%" }}>
        <SessionProgress now={12} planned={6} />
      </div>
    </CompSection>
  </CompFrame>
);
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
        ...
        <div style={{ minWidth: 0 }}>
          ...
          <div className="h1" style={{ fontWeight: 500 }}>
            Solid signal, room to land outcomes
          </div>
          <div
            className="small"
            style={{ marginTop: 6, display: "inline-flex", alignItems: "center", gap: 8 }}
          >
            <Chip kind={completion === "user_stopped" ? null : "ok"} dot>
              {reasonLabel}
            </Chip>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 }}>
        <div>
          <div className="micro upper" style={{ color: "var(--ok)", marginBottom: 8 }}>
            Strengths
          </div>
        ...
        <div>
          <div className="micro upper" style={{ color: "var(--err)", marginBottom: 8 }}>
            Improve
          </div>
          <ul style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            <li className="body" style={{ paddingLeft: 14, position: "relative" }}>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        ...
        <Button kind="ghost" icon="book">
          View transcript
        </Button>
        <Button kind="quiet" icon="download" style={{ marginLeft: "auto" }}>
          Export
        </Button>
      </div>
    </div>
...
const CompResultSummary = () => (
  <CompFrame kicker="Component" title="ResultSummary">
    <div className="body">
      Used at the top of every Result page. Score gauge, completion reason, strengths &
      improvements, action row.
    </div>
