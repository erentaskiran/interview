...
const TEST_DATABASE_URL = process.env.TEST_DATABASE_URL || "postgresql://ainterview:ainterview@localhost:5432/ainterview_test";
...
export async function createTestUser(
  ...
  return JSON.parse(response.body) as { token: string; user: { id: string; email: string; displayName: string } };
}

export async function createTestTemplate(
  app: Awaited<ReturnType<typeof buildTestApp>>,
  token: string,
  data?: Partial<{ title: string; category: string; description: string; systemInstruction: string }>
) {
  const payload = {
    title: data?.title || "Test Template",
    category: data?.category || "Engineering",
    description: data?.description || "A test template description that is long enough.",
    systemInstruction: data?.systemInstruction || "You are a test interviewer. Ask relevant questions.",
